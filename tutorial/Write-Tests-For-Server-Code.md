...
There are multiple uses of the `TEST` macro in the unit test [parsed\_projection\_test.cpp](https://github.com/mongodb/mongo/blob/v2.6/src/mongo/db/query/parsed_projection_test.cpp#L91) found in the `src/mongo/db/query` subdirectory. To build parsed\_projection\_test.cpp, it must be [declared inside an SConscript file](https://github.com/mongodb/mongo/blob/v2.6/src/mongo/db/query/SConscript#L178) found in the same `src/mongo/db/query` subdirectory.
